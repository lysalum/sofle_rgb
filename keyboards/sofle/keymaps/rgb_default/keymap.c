 ...
 * `-----------------------------------------/       /     \      \-----------------------------------------'
 *            | LCTR | WIN  |LOWER | Enter| /Enter  /       \SPACE \  |SPACE |RAISE | RCTR | RAlt |
 *            |      |      |      |      |/       /         \      \ |      |      |      |      |
 *            `----------------------------------'           '------''---------------------------'
 */
  [_QWERTY] = LAYOUT(
  ...
  //|------+-------+--------+--------+--------+------|  ===  |   |  ===  |--------+-------+--------+--------+--------+---------|
                 KC_LCTL, KC_LGUI, KC_LOWER, KC_ENT,  KC_ENT   ,     KC_SPC, KC_SPC ,  KC_RAISE, KC_RCTL, KC_RALT
  //            \--------+--------+--------+---------+-------|   |--------+---------+--------+---------+-------/
),
...
[_RAISE] = LAYOUT(
  ...
  //|------+-------+--------+--------+--------+------|                   |--------+-------+--------+--------+--------+---------|
  _______,  KC_INS,  KC_PSCR,   KC_APP,  XXXXXXX, XXXXXXX,                KC_CIRC, KC_AMPR,  KC_UP, KC_LCBR, KC_RCBR, KC_BSPC,
  //|------+-------+--------+--------+--------+------|                   |--------+-------+--------+--------+--------+---------|
...
#define SET_LAYER_ID(hsv) 	\
	{0, 72, hsv},    \
    SET_INDICATORS(HSV_CORAL),  \
    SET_UNDERGLOW(HSV_PINK),    \
    SET_THUMB_CLUSTER(HSV_GOLDENROD), \
    SET_NUMROW(HSV_PURPLE), \
    SET_OUTER_COL(HSV_AZURE)
...
// _NUMLOCK,
// Light on outer column and underglow
const rgblight_segment_t PROGMEM layer_numlock_lights[] = RGBLIGHT_LAYER_SEGMENTS(
	{36 + 31, 1, HSV_MAGENTA}
);
